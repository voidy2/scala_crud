...
object MyBuild extends Build {
  ...
  lazy val root = Project("root", file("."), 
    settings = Defaults.defaultSettings ++ webSettings ++ rootSettings)
  ...
  lazy val rootSettings = Seq(
    ...
    getPage <<= getPage.dependsOn(Keys.compile in Compile)
  )
  ...
  def checkPageTask = InputTask(_ => 
    complete.Parsers.spaceDelimited("<arg>"))  { 
    result =>
      (getPage, result) map {
        (gp, args) => checkHelloWorld(args.mkString(" ")) foreach error
      }
  }
